...
18b. ~~Write-conflict detection wired end-to-end~~ — **done** (2026-07-09, commit `bc99b39`): first-committer-wins existed in `TransactionManager` but could never trigger — no mutation path recorded write sets, and a failed commit was reported to clients as success. Fixed in 3 milestones (full detail in `tasks/todo.md`): (M1) commit results surface end-to-end — `StorageAccess::commit_tx` returns `Result<(), String>`, propagating through executor auto-commit (`ExecutionError::CommitFailed`), `Response::CommitResult`, `ShardCluster::commit_explicit_tx`, and the server's COMMIT_TX handler to wire clients as an ERROR; (M2) `Shard::log_page_write()` (the choke point every node/edge/prop record write funnels through) records write sets via `TransactionManager::record_write()` with conflict key `(page_type, page_id, slot)`; (M3) `ShardCluster` supports N concurrent explicit transactions — tx_id-parameterized `commit_explicit_tx(tx_id)`/`abort_explicit_tx(tx_id)`/`query_in_tx(tx_id, gql)`, no single-global-tx assert, auto-commit queries always allocate their own tx instead of silently joining an open explicit tx, server passes per-connection `active_tx` everywhere including orphan-abort on disconnect. 319 tests passing. Two documented adjacent gaps, deliberately not built: cross-shard commit atomicity (a multi-shard conflicting tx commits on non-conflicting shards while the client gets an error — two-phase-commit fix sketch in `tasks/todo.md` Milestone 4, declined 2026-07-09) and property records having no MVCC versioning (overwritten in place, no tx_min/tx_max).
19. **Next**: semi-sync/sync replication (Phase 8f-durability), snapshot shipping (Phase 8g), automatic failover (Phase 8h); deferred correctness work: cross-shard commit atomicity (see 18b), property-record MVCC versioning
